...
abstract class ExpertPrimaryAffiliationQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = '\\CryoConnectDB\\ExpertPrimaryAffiliation', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = ExpertPrimaryAffiliationTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
     ...
     *
     * @return ChildExpertPrimaryAffiliation A model object, or null if the key is not found
     */
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        $sql = 'SELECT expert_id, primary_affiliation_name, primary_affiliation_country_code, primary_affiliation_city, timestamp FROM expert_primary_affiliation WHERE expert_id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            /** @var ChildExpertPrimaryAffiliation $obj */
            $obj = new ChildExpertPrimaryAffiliation();
            $obj->hydrate($row);
            ExpertPrimaryAffiliationTableMap::addInstanceToPool($obj, null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key);
        }
        $stmt->closeCursor();
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        return $this->addUsingAlias(ExpertPrimaryAffiliationTableMap::COL_EXPERT_ID, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(ExpertPrimaryAffiliationTableMap::COL_EXPERT_ID, $keys, Criteria::IN);
    }
    ...
    public function prune($expertPrimaryAffiliation = null)
    {
        if ($expertPrimaryAffiliation) {
            $this->addUsingAlias(ExpertPrimaryAffiliationTableMap::COL_EXPERT_ID, $expertPrimaryAffiliation->getExpertId(), Criteria::NOT_EQUAL);
        }
