...
abstract class ExpertCryosphereWhereQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = '\\CryoConnectDB\\ExpertCryosphereWhere', $modelAlias = null)
    {
     ...
     * </code>
     *
     * @param array[$expert_id, $cryosphere_where_id] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = ExpertCryosphereWhereTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]))))) {
     ...
     *
     * @return ChildExpertCryosphereWhere A model object, or null if the key is not found
     */
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        $sql = 'SELECT expert_id, cryosphere_where_id, timestamp FROM expert_cryosphere_where WHERE expert_id = :p0 AND cryosphere_where_id = :p1';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            /** @var ChildExpertCryosphereWhere $obj */
            $obj = new ChildExpertCryosphereWhere();
            $obj->hydrate($row);
            ExpertCryosphereWhereTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(ExpertCryosphereWhereTableMap::COL_EXPERT_ID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(ExpertCryosphereWhereTableMap::COL_CRYOSPHERE_WHERE_ID, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(ExpertCryosphereWhereTableMap::COL_EXPERT_ID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(ExpertCryosphereWhereTableMap::COL_CRYOSPHERE_WHERE_ID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    public function prune($expertCryosphereWhere = null)
    {
        if ($expertCryosphereWhere) {
            $this->addCond('pruneCond0', $this->getAliasedColName(ExpertCryosphereWhereTableMap::COL_EXPERT_ID), $expertCryosphereWhere->getExpertId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(ExpertCryosphereWhereTableMap::COL_CRYOSPHERE_WHERE_ID), $expertCryosphereWhere->getCryosphereWhereId(), Criteria::NOT_EQUAL);
