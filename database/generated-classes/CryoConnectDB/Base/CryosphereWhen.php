...
/**
 * Base class that represents a row from the 'cryosphere_when' table.
 *
 *
 *
 * @package    propel.generator.CryoConnectDB.Base
 */
abstract class CryosphereWhen implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\CryoConnectDB\\Map\\CryosphereWhenTableMap';
    ...
    /**
     * The value for the cryosphere_when_name field.
     *
     * @var        string
     */
    protected $cryosphere_when_name;
    ...
    /**
     * Initializes internal state of CryoConnectDB\Base\CryosphereWhen object.
     * @see applyDefaults()
     */
    ...
    /**
     * Compares this with another <code>CryosphereWhen</code> instance.  If
     * <code>obj</code> is an instance of <code>CryosphereWhen</code>, delegates to
     * <code>equals(CryosphereWhen)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|CryosphereWhen The current object, for fluid interface
     */
    ...
    /**
     * Get the [cryosphere_when_name] column value.
     *
     * @return string
     */
    public function getCryosphereWhenName()
    {
        return $this->cryosphere_when_name;
    }
     ...
     * @return $this|\CryoConnectDB\CryosphereWhen The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[CryosphereWhenTableMap::COL_ID] = true;
        }
    ...
    /**
     * Set the value of [cryosphere_when_name] column.
     *
     * @param string $v new value
     * @return $this|\CryoConnectDB\CryosphereWhen The current object (for fluent API support)
     */
    public function setCryosphereWhenName($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->cryosphere_when_name !== $v) {
            $this->cryosphere_when_name = $v;
            $this->modifiedColumns[CryosphereWhenTableMap::COL_CRYOSPHERE_WHEN_NAME] = true;
        }

        return $this;
    } // setCryosphereWhenName()
    ...
    public function setApproved($v)
    {
        ...
        if ($this->approved !== $v) {
            $this->approved = $v;
            $this->modifiedColumns[CryosphereWhenTableMap::COL_APPROVED] = true;
        }
    ...
    public function setTimestamp($v)
    {
        ...
        if ($this->timestamp !== null || $dt !== null) {
            if ($this->timestamp === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->timestamp->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[CryosphereWhenTableMap::COL_TIMESTAMP] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : CryosphereWhenTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : CryosphereWhenTableMap::translateFieldName('CryosphereWhenName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->cryosphere_when_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CryosphereWhenTableMap::translateFieldName('Approved', TableMap::TYPE_PHPNAME, $indexType)];
            $this->approved = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CryosphereWhenTableMap::translateFieldName('Timestamp', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = CryosphereWhenTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\CryoConnectDB\\CryosphereWhen'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildCryosphereWhenQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see CryosphereWhen::setDeleted()
     * @see CryosphereWhen::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildCryosphereWhenQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                CryosphereWhenTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[CryosphereWhenTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . CryosphereWhenTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(CryosphereWhenTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(CryosphereWhenTableMap::COL_CRYOSPHERE_WHEN_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'cryosphere_when_name';
        }
        if ($this->isColumnModified(CryosphereWhenTableMap::COL_APPROVED)) {
            $modifiedColumns[':p' . $index++]  = 'approved';
        }
        if ($this->isColumnModified(CryosphereWhenTableMap::COL_TIMESTAMP)) {
            $modifiedColumns[':p' . $index++]  = 'timestamp';
        }

        $sql = sprintf(
            'INSERT INTO cryosphere_when (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':
                        ...
                        break;
                    case 'cryosphere_when_name':
                        $stmt->bindValue($identifier, $this->cryosphere_when_name, PDO::PARAM_STR);
                        break;
                    case 'approved':
    ...
    {
        $pos = CryosphereWhenTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function getByPosition($pos)
    {
        switch ($pos) {
            case 0:
                ...
                break;
            case 1:
                return $this->getCryosphereWhenName();
                break;
            case 2:
                return $this->getApproved();
                break;
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['CryosphereWhen'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['CryosphereWhen'][$this->hashCode()] = true;
        $keys = CryosphereWhenTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getCryosphereWhenName(),
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\CryoConnectDB\CryosphereWhen
     */
    ...
    public function setByPosition($pos, $value)
    {
        switch ($pos) {
            case 0:
                ...
                break;
            case 1:
                $this->setCryosphereWhenName($value);
                break;
            case 2:
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setCryosphereWhenName($arr[$keys[1]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\CryoConnectDB\CryosphereWhen The current object, for fluid interface
     */
    ...
    {
        $criteria = new Criteria(CryosphereWhenTableMap::DATABASE_NAME);

        if ($this->isColumnModified(CryosphereWhenTableMap::COL_ID)) {
            $criteria->add(CryosphereWhenTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(CryosphereWhenTableMap::COL_CRYOSPHERE_WHEN_NAME)) {
            $criteria->add(CryosphereWhenTableMap::COL_CRYOSPHERE_WHEN_NAME, $this->cryosphere_when_name);
        }
        if ($this->isColumnModified(CryosphereWhenTableMap::COL_APPROVED)) {
            $criteria->add(CryosphereWhenTableMap::COL_APPROVED, $this->approved);
        }
        if ($this->isColumnModified(CryosphereWhenTableMap::COL_TIMESTAMP)) {
            $criteria->add(CryosphereWhenTableMap::COL_TIMESTAMP, $this->timestamp);
        }
    ...
    {
        $criteria = ChildCryosphereWhenQuery::create();
        $criteria->add(CryosphereWhenTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \CryoConnectDB\CryosphereWhen (or compatible) type.
    ...
    {
        $copyObj->setCryosphereWhenName($this->getCryosphereWhenName());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \CryoConnectDB\CryosphereWhen Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildCryosphereWhen is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildCryosphereWhen The current object (for fluent API support)
     */
    public function setExpertCryosphereWhens(Collection $expertCryosphereWhens, ConnectionInterface $con = null)
    {
        ...
        foreach ($expertCryosphereWhensToDelete as $expertCryosphereWhenRemoved) {
            $expertCryosphereWhenRemoved->setCryosphereWhen(null);
        }
    ...
    protected function doAddExpertCryosphereWhen(ChildExpertCryosphereWhen $expertCryosphereWhen)
    {
        ...
        $expertCryosphereWhen->setCryosphereWhen($this);
    }
    ...
    public function removeExpertCryosphereWhen(ChildExpertCryosphereWhen $expertCryosphereWhen)
    {
        if ($this->getExpertCryosphereWhens()->contains($expertCryosphereWhen)) {
            ...
            $expertCryosphereWhen->setCryosphereWhen(null);
        }
     ...
     * Otherwise if this CryosphereWhen is new, it will return
     * an empty collection; or if this CryosphereWhen has previously
     * been saved, it will retrieve related ExpertCryosphereWhens from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in CryosphereWhen.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @return ObjectCollection|ChildExpertCryosphereWhen[] List of ChildExpertCryosphereWhen objects
     */
    public function getExpertCryosphereWhensJoinExperts(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildExpertCryosphereWhenQuery::create(null, $criteria);
     ...
     * @return ChildCryosphereWhen The current object (for fluent API support)
     */
    ...
    protected function doAddExperts(ChildExperts $experts)
    {
        ...
        $expertCryosphereWhen->setExperts($experts);
        ...
        if (!$experts->isCryosphereWhensLoaded()) {
            $experts->initCryosphereWhens();
            $experts->getCryosphereWhens()->push($this);
        } elseif (!$experts->getCryosphereWhens()->contains($this)) {
            $experts->getCryosphereWhens()->push($this);
        }
    ...
    public function removeExperts(ChildExperts $experts)
    {
        if ($this->getExpertss()->contains($experts)) {
            ...
            if ($experts->isCryosphereWhensLoaded()) {
                ...
                $experts->getCryosphereWhens()->removeObject($this);
            }
    ...
    {
        $this->id = null;
        $this->cryosphere_when_name = null;
        $this->approved = null;
        $this->timestamp = null;
        $this->alreadyInSave = false;
        $this->clearAllReferences();
    ...
    {
        return (string) $this->exportTo(CryosphereWhenTableMap::DEFAULT_STRING_FORMAT);
    }
